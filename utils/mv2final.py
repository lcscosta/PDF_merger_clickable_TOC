...
for folder in subfolders:
    ...
    if len(files) == 1 and len(glob.glob(os.path.join(folder,"*.pdf"))) == 1:
        ...
        print(files[0])
        newname = folder.split('Trabalho Completo-20240816T181335Z-001/Trabalho Completo/')[1]
        shutil.copy2(files[0], f'./Final/{newname}.pdf')
    if len(files) == 1 and len(glob.glob(os.path.join(folder,"*.docx"))) == 1:
        count_docx += 1
        path_docx.append(folder)
        newname = folder.split('Trabalho Completo-20240816T181335Z-001/Trabalho Completo/')[1]
        shutil.copy2(files[0], f'./Final/{newname}.docx')
